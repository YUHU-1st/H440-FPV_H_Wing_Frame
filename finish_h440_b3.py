...
import ezdxf
import vtk
O=R/'H440_B3';report=json.loads((O/'geometry_report.json').read_text(encoding='utf-8'))
hardware=[
 ('H01','M3×8钢螺钉：碳板到法兰套',16,.65),
 ('H02','M3×12钢螺钉：电机板16＋图传转接4',20,.9),
 ('H03','M3×22钢螺钉：电机U座四孔夹侧板，按垫片实测选长',16,1.3),
 ('H04','M3×12钢螺钉：TPU相机侧板上下各四点',8,.9),
 ...
 ('H17','M3×75横向贯穿螺栓，实际螺纹与尾端伸出需核对',4,4.3),
 ('H18','TPU固定脚铝合金限压套OD5/ID3.2×3.0mm',8,.1),
 ('H19','TPU固定脚M3大垫片OD8，覆盖限压套与TPU',8,.2)]
...
frame=cq.importers.importStep(str(O/'H440_B3_FRAME_multibody.step'))
travel=[]
for z in [-105,-75,-45]:
...
camdeck=cq.importers.importStep(str(O/'C04_electronics_floor_2mm.step')).union(cq.importers.importStep(str(O/'C03_battery_floor_2mm.step')))
...
for i in range(361):
    ...
    camclear.append(camera.val().distance(camdeck.val()))
...
lines=['# H440 B3 机架 BOM 与重量预算','',f'标准配置：PLA机翼、ABS连接件、TPU95A双端相机侧板、2/4mm碳板、两根碳管、四根铝隔柱。CAD结构净重 **{report["CAD_frame_mass_g"]:.1f} g**；紧固件、绑带、防滑垫与胶估计 **{hw:.1f} g**；机架合计 **{report["frame_estimate_g"]:.1f} g**。建议按 **680 g** 留重量预算，首件称重前估计范围约 **600–720 g**。','',
...
'## 材料口径与采购下料','',
'- PLA按1.24、ABS按1.04、TPU按1.21、铝合金按2.70、碳板/碳管按1.60 g/cm³预算；均须用实际耗材和称重核对。',
...
def shape_actor(s,col):
    ...
    return a
battery_actor=shape_actor(cq.Workplane('XY').box(48,52,78).translate((0,47,-75)),(.85,.73,.12))
for x in [-145,145]:
    for y in [-78,78]:
...
tx=vtk.vtkTextActor();tx.SetInput(f'H440 B3 / HYBRID\nPLA wings + CFRP frame + TPU camera sides\nCAD structure {report["CAD_frame_mass_g"]:.0f} g; frame estimate {report["frame_estimate_g"]:.0f} g')
...
for name,pos,scale in [('H440_B3_isometric.png',(480,340,550),265),('H440_B3_front.png',(0,0,800),240)]:
...
battery_actor.SetVisibility(False)
for n,a in actors.items():
    if n.startswith(('W','Motor','Pylon','Shoe')) or 'pylon' in n:a.SetVisibility(False)
collection=rn.GetActors();collection.InitTraversal()
for _ in range(collection.GetNumberOfItems()):
    a=collection.GetNextActor()
    if a not in actors.values():a.SetVisibility(False)
actors['Battery_floor'].GetProperty().SetOpacity(.45)
cam.SetFocalPoint(0,-1,-49);cam.SetPosition(130,95,135);cam.SetParallelScale(93)
tx.SetInput('H440 B3 / CENTER DETAIL\nMetal spacers + shouldered decks + dual-ended TPU sides\nBattery deck translucent; fasteners listed in BOM')
rn.ResetCameraClippingRange();rw.Render()
cap=vtk.vtkWindowToImageFilter();cap.SetInput(rw);cap.Update();wr=vtk.vtkPNGWriter();wr.SetFileName(str(O/'H440_B3_center_detail.png'));wr.SetInputConnection(cap.GetOutputPort());wr.Write()
rw.Finalize()
