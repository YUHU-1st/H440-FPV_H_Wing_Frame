"""H440 B1 hybrid frame. mm; X span, Y up, Z forward.
...
parts={};instances=[]
colors={'PLA':(.08,.57,.68),'ABS':(.95,.43,.12),'CFRP':(.15,.18,.21)}
rho={'PLA':1.24,'ABS':1.04,'CFRP':1.60}
...
side=box(2,60,158,32,43,-74).cut(box(4,52,150,32,43,-74))
for z in [-44,-98]:
    side=side.union(box(2,42,26,32,1,z))
    side=side.union(box(2,60,5,32,43,z))
    ...
    # Deck / battery corner-bracket bolt axes X.
    side=side.cut(cx(1.7,5,30,23,z+9))
    side=side.cut(cx(1.7,5,30,-12,z+17))
side=reg('C01_center_side_R_2mm',side,'CFRP',description='中央右侧板，2mm碳板',axis='X')
...
for x in [-23,23]:
    for z in [-35,-89]:floor=floor.cut(cy(1.7,5,x,19,z))
...
for x in [-28,28]:
    for z in [-27,-81]:deck=deck.union(box(6,2,12,x,-19,z))
...
# L brackets connect horizontal floors to side plates, M3 cross-axis fasteners.
br=box(12,3,10,25,18.5,-35).union(box(3,12,10,29.5,23,-35))
br=br.cut(cy(1.7,8,23,22,-35)).cut(cx(1.7,6,27,23,-35))
br=reg('P01_floor_angle',br,'ABS',8,'通用托板角码，4电池+4电子',axis='Z')
for zoff in [0,-54]:
    for sign in [1,-1]:
        s=br if sign==1 else br.mirror('YZ')
        add(f'Battery_bracket_{zoff}_{sign}',s.translate((0,0,zoff)),'ABS')
        e=s.translate((0,-35,8))
        add(f'Electronics_bracket_{zoff}_{sign}',e.translate((0,0,zoff)),'ABS')
...
for n,r,z in [('P02_main_collar',5.15,-44),('P03_rear_collar',3.15,-98)]:
    ...
    for xoff,label in [(0,'center'),(112.5,'pylon')]:
        a=col.translate((xoff,0,0));b=a.mirror('YZ')
        add(n+'_'+label+'_R',a,'ABS');add(n+'_'+label+'_L',b,'ABS');collars.extend([a,b])

# 3mm side truss: one upper+lower plate per side; motor loads in plate plane.
pl=cq.Plane(origin=(143.5,0,0),xDir=(0,0,1),normal=(1,0,0))
def yzpoly(pts,th=3):return cq.Workplane(pl).polyline([(z,-y) for y,z in pts]).close().extrude(th)
def bar(a,b,w=6):
...
upper=bar(A,C).union(bar(B,D)).union(bar(C,D)).union(bar(B,C,5))
truss=upper.union(upper.mirror('XZ')).union(bar(A,B,14))
for z in [-44,-98]:truss=truss.union(cx(12,3,143.5,0,z))
for y in [-78,78]:truss=truss.union(box(3,26,16,145,y,0))
for y in [-83,83]:truss=truss.union(box(3,12,8,145,y,-161))
for z,r in [(-44,5.2),(-98,3.2)]:
    truss=truss.cut(cx(r,5,142,0,z))
    for y in [-8,8]:truss=truss.cut(cx(1.7,5,142,y,z))
for y0 in [-78,78]:
    for dy in [-7,7]:truss=truss.cut(cx(1.7,5,142,y0+dy,0))
truss=reg('C05_pylon_R_3mm',truss,'CFRP',description='右侧上下联体承力桁架，3mm',axis='X')
trussL=reg('C06_pylon_L_3mm',truss.mirror('YZ'),'CFRP',description='左侧上下联体承力桁架，3mm',axis='X')
...
for dx in [-4.6,4.6]:ad=ad.union(box(6,24,16,145+dx,78,0))
ad=ad.cut(cx(1.7,20,135,71,0)).cut(cx(1.7,20,135,85,0))
for dx in [-8,8]:
    for dy in [-8,8]:ad=ad.cut(cz(3.0,23,145+dx,78+dy,-9))
for dx in [-12,12]:
    for dy in [-12,12]:ad=ad.cut(cz(1.7,6,145+dx,78+dy,7))
ad=reg('P04_motor_clevis',ad,'ABS',4,'电机碳板与垂直侧板U形转接座',axis='Z')
for x in [-145,145]:
    for y in [-78,78]:
...
shoe=box(14,18,8,145,83,-165).cut(box(3.3,20,5,145,83,-162.5))
shoe=reg('P05_tail_shoe',shoe,'ABS',4,'可更换着陆脚套，胶接3mm侧板边缘',axis='X')
for x in [-145,145]:
...
wi=wing.intersect(box(109.3,50,160,88.65,0,-75))
wo=wing.intersect(box(73.3,50,160,183.35,0,-75))
...
report={'revision':'B1','status':'GEOMETRY CHECKED; NO STRUCTURAL OR FLIGHT VALIDATION','density_g_cm3':rho,'parts':{},'checks':{}}
frame=cq.Assembly(name='H440_B1_FRAME')
...
frame.export(str(O/'H440_B1_FRAME.step'))
layout=cq.Assembly(name='H440_B1_LAYOUT');layout.add(frame,name='Frame')
refs=[]
...
layout.export(str(O/'H440_B1_LAYOUT.step'))
# Also provide a single multi-body STEP for a robust SolidWorks part import.
cq.exporters.export(cq.Compound.makeCompound([s.val() for _,s,_ in instances]),str(O/'H440_B1_FRAME_multibody.step'))
ints=[]
