"""H440 B2 hybrid frame. mm; X span, Y up, Z forward.
...
O=R/'H440_B2';O.mkdir(exist_ok=True)
...
report={'revision':'B2','status':'GEOMETRY CHECKED; NO STRUCTURAL OR FLIGHT VALIDATION','density_g_cm3':rho,'parts':{},'checks':{}}
frame=cq.Assembly(name='H440_B2_FRAME')
...
frame.export(str(O/'H440_B2_FRAME.step'))
layout=cq.Assembly(name='H440_B2_LAYOUT');layout.add(frame,name='Frame')
refs=[]
...
layout.export(str(O/'H440_B2_LAYOUT.step'))
# Also provide a single multi-body STEP for a robust SolidWorks part import.
cq.exporters.export(cq.Compound.makeCompound([s.val() for _,s,_ in instances]),str(O/'H440_B2_FRAME_multibody.step'))
ints=[]
